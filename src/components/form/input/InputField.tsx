...
export default function InputField({ className = "", error, success, hint, ...props }: InputProps) {
  ...
  return (
    <div className="relative w-full">
      <input
        className={`w-full rounded-lg border bg-white px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30 ${borderClass} ${className}`}
        {...props}
      />
      {hint && (
