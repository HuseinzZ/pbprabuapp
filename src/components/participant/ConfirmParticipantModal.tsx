...
export default function ConfirmParticipantModal({ isOpen, participantName, onClose, onConfirm, isConfirming }: ConfirmModalProps) {
    ...
    <div className="fixed inset-0 z-50 overflow-y-auto flex items-center justify-center p-4 bg-slate-950/60 backdrop-blur-sm transition-opacity duration-200">
      ...
      >
        ...
        <div className="flex flex-col items-center text-center space-y-3">
          <div className="p-3 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 rounded-lg border border-emerald-200 dark:border-emerald-500/20">
          ...
          <div>
            ...
            <p className="text-xs text-slate-500 dark:text-gray-400 mt-1 max-w-xs mx-auto">
              Apakah Anda yakin ingin mengkonfirmasi peserta <strong className="text-slate-700 dark:text-gray-300">"{participantName}"</strong>? Peserta ini akan berstatus aktif.
            </p>
          </div>
        </div>
        ...
        <div className="flex gap-3 pt-2">
          ...
          <button
            onClick={onConfirm}
            disabled={isConfirming}
            className="flex-1 px-4 py-2 bg-emerald-600 hover:bg-emerald-700 text-white text-sm font-semibold rounded-lg transition-colors flex items-center justify-center gap-2 disabled:opacity-50"
          >
            {isConfirming ? (
              <>
