...
import { uploadStorageFile } from '@/lib/utils/supabaseStorage';
...
export default function AddCarouselPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // 1. Upload image to 'gallery' bucket
      const uploadedPath = await uploadStorageFile(file, 'gallery');

      // 2. Save data to 'carousels' table
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex flex-col gap-4">
        <PageBreadcrumb pageTitle="Tambah Banner Beranda" />
      </div>
      ...
      <div className="bg-white dark:bg-gray-900 border border-slate-200 dark:border-gray-800 rounded-xl shadow-sm overflow-hidden">
        <form onSubmit={handleSubmit} className="p-6 md:p-8 space-y-6">
          <div className="space-y-4">
            ...
            {/* Image Upload */}
            <div>
              <label className="block text-xs font-bold text-slate-700 dark:text-gray-300 uppercase mb-2">Gambar Banner <span className="text-red-500">*</span></label>
